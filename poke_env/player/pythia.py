...
from poke_env.player.prompts import get_number_turns_faint, get_status_num_turns_fnt, state_translate
...
class Pythia(Player):
    ...
    def get_LLM_action(self, system_prompt, user_prompt, model, temperature=0.7, json_format=False, seed=None, stop=[], max_tokens=200, actions=None, llm=None, extra_info=None) -> str:
        if llm is None:
            output, _ = self.llm.get_LLM_action(system_prompt, user_prompt, model, temperature, True, seed, stop, max_tokens=max_tokens, actions=actions, extra_info=extra_info)
        else:
            output, _ = llm.get_LLM_action(system_prompt, user_prompt, model, temperature, True, seed, stop, max_tokens=max_tokens, actions=actions, extra_info=extra_info)
        return output
    ...
    def io(self, retries, system_prompt, state_prompt, constraint_prompt_cot, constraint_prompt_io, state_action_prompt, battle: Battle, sim, dont_verify=False, actions=None, extra_info=None):
    ...
    def tree_search(self, retries, battle, sim=None, return_opp = False) -> BattleOrder:
        ...
        while len(q) != 0:
            ...
            if len(node.simulation.battle.available_switches) != 0:
                ...
                for i in range(2):
                    action_llm_switch = self.io(retries, system_prompt, state_prompt, constraint_prompt_cot, constraint_prompt_io, state_action_prompt_switch, node.simulation.battle, node.simulation, extra_info="FORCE SWITCH")
            ...
            if not node.simulation.battle.active_pokemon.fainted and len(battle.available_moves) > 0:
                ...
                action_llm_move = self.io(retries, system_prompt, state_prompt, constraint_prompt_cot, constraint_prompt_io, state_action_prompt_move, node.simulation.battle, node.simulation, extra_info="FORCE MOVE")
                if len(player_actions) == 0:
            ...
            action_o = self.io(2, system_prompt_o, state_prompt_o, constraint_prompt_cot_o, constraint_prompt_io_o, state_action_prompt_o, node.simulation.battle, node.simulation, dont_verify=True, extra_info="OPPO BEST MOVE")
