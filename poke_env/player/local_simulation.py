...
def calculate_move_type_damage_multipier(
    ...
    if type_2:
        ...
        if type_1 == "STELLAR":
            move_type_damage_multiplier_dict = {
                "BUG": 1,
                "DARK": 1,
                "DRAGON": 1,
                "ELECTRIC": 1,
...
class LocalSim:
    ...
    def get_llm_system_prompt(
        self,
        _format: str,
        llm: GPTPlayer | LLAMAPlayer = None,
        team_str: str = None,
        model: str = "gpt-4o",
    ):
            ...
            if llm is not None:
                ...
                for poke_str in self.battle.team.keys():
                    ...
                    try:
                        if mon.item:
                            strategy_prompt += (
                                f" @ {self.item_effect[mon.item]['name']}"
                            )
                    except:
                        pass
                    strategy_prompt += "\n"
                    if mon.ability:
                        strategy_prompt += f"Ability: {mon.ability}\n"
                    # if self.gen.gen == 9:
                ...
                self.strategy, _ = llm.get_LLM_query(
                    "", strategy_prompt, max_tokens=1000, model=model
                )
                # print()
                # print(self.strategy)
    ...
    def step_llm(
        self, action1, action2, llm, model, m1, m2, temperature=0.7, max_tokens=200
    ):
        ...
        opponent_hp_fraction = round(
            self.battle.opponent_active_pokemon.current_hp
            / self.battle.opponent_active_pokemon.max_hp
            * 100
        )
        opponent_stats = self.battle.opponent_active_pokemon.calculate_stats(
        ...
        active_hp_fraction = round(
            self.battle.active_pokemon.current_hp
            / self.battle.active_pokemon.max_hp
            * 100
        )
        ...
        if self.battle.active_pokemon.type_1:
            ...
            if self.battle.active_pokemon.type_2:
                active_type = (
                    active_type
                    + " and "
                    + self.battle.active_pokemon.type_2.name.capitalize()
                )

        active_move_type_damage_prompt = move_type_damage_wrapper(
            self.battle.active_pokemon, self.gen.type_chart, opponent_type_list
        )
        speed_active_stats = active_stats["spe"]
        if speed_active_stats == None:
            speed_active_stats = 0
        active_speed = round(
            speed_active_stats * self.boost_multiplier("spe", active_boosts["spe"])
        )

        try:
            active_ability = self.ability_effect[self.battle.active_pokemon.ability][
                "name"
            ]
            ability_effect = self.ability_effect[self.battle.active_pokemon.ability][
                "effect"
            ]
        except:
        ...
        if self.battle.active_pokemon.item:
            try:
                ...
                item_effect = self.item_effect[self.battle.active_pokemon.item][
                    "effect"
                ]
        ...
        active_pokemon_prompt = (
            ...
            + (
                f"(slower than {self.battle.opponent_active_pokemon.species})."
                if active_speed < opponent_speed
                else f"(faster than {self.battle.opponent_active_pokemon.species})."
            )
            + (
        ...
        if "p1" in list(self.battle.team.keys())[0]:
            ...
            ).replace("p1a: ", "player:").replace("p2a:", "opponent:").replace(
                "Player1", "Player"
            ).replace(
                "Player2", "Opponent"
            )
        ...
        if DEBUG:
            print(f"OPPONENT {opponent_tag}")
        action1_name = action1.split(" ")[-1].title()
        if DEBUG:
            print("action 1 name", action1_name)
        ...
        if "switch" in action2:
            ...
            msg = [
                "",
                "switch",
                f"{opponent_tag}: {action2_name}",
                "",
                f"{opponent_health}/100",
            ]
            msg_all.append(msg)
        ...
        if DEBUG:
            print(self.battle.active_pokemon, self.battle.opponent_active_pokemon)
        if DEBUG:
            for avail_mon in self.battle.opponent_team.values():
                print("opp", avail_mon)
        assert self.battle.opponent_active_pokemon != None
        player_health, opponent_health, m1_success, m2_success = (
            self.calculate_remaining_hp(
        ...
        if "move" in action1:
            msg = [
                "",
                "move",
                f"{player_tag}: {self.battle.active_pokemon.species.title()}",
                f"{action1_name}",
                f"{opponent_tag}: {self.battle.opponent_active_pokemon.species.title()}",
            ]
            msg_all.append(msg)
    ...
    def step(self, action1: BattleOrder, action2: BattleOrder):
        ...
        if action2 is not None:
            if "switch" in action2 and not "move" in action2:
                ...
                opponent_health = 100
                for avail_mon in self.battle.opponent_team.values():
                    if avail_mon.species == action2_name.lower().replace(
                        " ", ""
                    ).replace("-", ""):
                        opponent_health = int(avail_mon.current_hp_fraction * 100)
    ...
    def get_player_prompt(self, return_actions=False, return_choices=False):
        ...
        if return_actions:
            (
                ...
                self, self.battle, return_actions=return_actions
            )  # add lower case
        elif return_choices:
            (
                system_prompt,
                ...
                self, self.battle, return_choices=return_choices
            )  # add lower case
        else:
            system_prompt, state_prompt, state_action_prompt = self.prompt_translate(
                self, self.battle
            )  # add lower case
    ...
    def get_opponent_possible_mons(self):
        ...
        if self.battle.opponent_active_pokemon.moves:
            for (
                move_id,
                opponent_move,
            ) in self.battle.opponent_active_pokemon.moves.items():
                opponent_moves.append(f"{opponent_move}")

    def get_opponent_current_moves(
        self, mon=None, return_switch=False, is_player=False, return_separate=False
    ):
        if is_player:
        ...
        if self.format == "gen9ou":
            try:
                possible_moves = [
                    move_set["name"].lower().replace(" ", "").replace("-", "")
                    for move_set in self.moves_set[species]["moves"]
                ]
            except:
    ...
    def get_turn_summary(self, battle: Battle, n_turn: int = 5) -> str:
    ...
    def get_opponent_prompt(self, state_prompt, return_actions=False):
        ...
        if len(observable_switches) > 0:
            swich_prompt += (
                f"[<opponent_switch_pokemon_name>] = {observable_switches}\n"
            )
    ...
    def apply_protosynthesis(self, mon: Pokemon, returned_stat):
        if (
            mon.ability == "protosynthesis" or mon.ability == "quarkdrive"
        ) and mon.item == "boosterdrive":
    ...
    def calculate_remaining_hp(
        ...
        if m1 != None:
            ...
            if m1.category != MoveCategory.STATUS:
                d1 = self.calc_base_dmg(
                    p1, p2, m1, boosts1=boosts1, boosts2=boosts2, team=team
                )
        ...
        if m2 != None:
            ...
            if m2.category != MoveCategory.STATUS:
                d2 = self.calc_base_dmg(
                    p2, p1, m2, boosts1=boosts2, boosts2=boosts1, team=opp_team
                )
        ...
        p1_speed = round(
            stats1["spe"] * self.boost_multiplier("spe", boosts1["spe"])
    ...
    def modify_base_power(
        self, mon: Pokemon, target: Pokemon, move: Move, team=None
    ) -> float:
    ...
    def apply_item(self, mon: Pokemon, boosts: Dict[str, int]) -> Dict[str, int]:
        ...
        if mon.ability in ["purepower", "hugepower"]:
            boost("atk", 2.0)
        elif mon.ability == "hustle" or (
            mon.ability == "gorillatactics" and not mon.is_dynamaxed
        ):
            boost("atk", 1.5)
        ...
        if move.category == MoveCategory.PHYSICAL:
            A = (
                ...
                if active_boosts["atk"] == 0
                    ...
                    stats["atk"] * self.boost_multiplier("atk", active_boosts["atk"])
                )
            )
            ...
            D = (
                ...
                if target_boosts["def"] == 0
                    ...
                    stats_target["def"]
                    * self.boost_multiplier("def", target_boosts["def"])
                )
            )
            ...
            A = (
                ...
                if active_boosts["spa"] == 0
                    ...
                    stats["spa"] * self.boost_multiplier("spa", active_boosts["spa"])
                )
            )
            ...
            D = (
                ...
                if target_boosts["spd"] == 0
                    ...
                    stats_target["spd"]
                    * self.boost_multiplier("spd", target_boosts["spd"])
                )
            )
            D = D * self.apply_protosynthesis(pokemon, "spd")
        assert A != -1
        baseDamage += (((2 * level) / 5.0 + 2) * power * A / D) / 50.0 + 2
        return baseDamage
    ...
    def modify_damage(
        ...
        if target.ability == "voltabsorb" and move.type == "electric":
            baseDamage *= 0
        if (
            target.ability == "waterabsorb" or target.ability == "dryskin"
        ) and move.type == "water":
            baseDamage *= 0
        ...
        if target.ability == "dryskin" and move.type == "fire":
            baseDamage *= 1.25
        if target.ability == "wonderguard" and not (
            extreme_effective_type_list or effective_type_list
        ):
            baseDamage *= 0
        ...
        if (
            ...
            and not pokemon.ability == "guts"
        ):
            if self.gen.gen < 6 or move.id != "facade":
                baseDamage *= 0.5

        # Generation 5, but nothing later, sets damage to 1 before the final damage modifiers
        if self.gen.gen == 5 and baseDamage == 0:
            baseDamage = 1
    ...
    def _handle_battle_message(self, split_message: List[str]):
        ...
        if split_message[1] == "switch":
            ...
            description = (
                " "
                + split_message[2].split(" ")[0]
                + " sent out "
                + split_message[2].split(": ")[-1]
                + "."
            )
            ...
            description = (
                " " + split_message[2] + " cannot move because of " + reason + "."
            )
            ...
            description = (
                " Field start: " + split_message[2] + " ran across the battlefield."
            )

        elif split_message[1] == "-fieldend":
            description = (
                " Field end: " + split_message[2] + " disappeared from the battlefield."
            )

        elif split_message[1] == "-ability":
            description = (
                " " + split_message[2] + "'s ability: " + split_message[3] + "."
            )
            ...
            try:
                previous_hp = battle.pokemon_hp_log_dict[split_message[2]][-1].split(
                    " "
                )[0]
            except:
                previous_hp = "100/100"
            ...
            description = (
                " " + split_message[2] + " activated " + split_message[3] + "."
            )
