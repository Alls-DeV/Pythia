...
def eval_action_player(llm: LLMPlayer, gen: int = 9, format: str = "randombattle"):
    ...
    for i in pbar:
        ...
        pbar.set_description(
            f"Correct moves: {correct_moves/total_moves*100:.2f}% = {correct_moves}/{total_moves}. Expected: {action} vs. Output: {output}"
        )
    return
...
def get_loadout_str(data, mon):
    ...
    def get_weighted_choice(category, id, size=1):
        ...
        p = np.array(
            [
                float(category_dict[i]["percentage"]) / 100.0
        ...
        if category == "moves":
            out = [
                item[i][id] if item[i][id] != "Nothing" else ""
...
async def one_vs_one(args, PNUMBER1, total=1, compare_bots=False):
    ...
    for i in pbar:
        ...
        if compare_bots:
            pbar.set_description(
                f"Winrate: {p2.win_rate*100:.2f}. Missed Wins: {missed_wins}. Bot wins {p2_bot.win_rate*100:.2f} heuristic: {p3.win_rate*100:.2f} m: {missed_wins_p3}"
            )
        else:
            pbar.set_description(
                f"Winrate: {p2.win_rate*100:.2f}. Missed Wins: {missed_wins}. Bot wins {p2_bot.win_rate*100:.2f}"
            )
        # update counts
...
def send_to_llm2(llm: LLMPlayer, system_prompt: str, prompt: str, action: str) -> int:
    # get llm action
    # print(prompt)
    output = llm.get_LLM_action(
        system_prompt,
...
def send_to_llm(
    ...
    outputs = llm.llm.get_LLM_action_topK(
        system_prompt,
        prompt,
        "",
        actions=action_list,
...
def send_to_llm_po(
    ...
    outputs, outputs_opp = llm.llm.get_LLM_action_topK(
        system_prompt,
        prompt,
        "",
        actions=action_list,
        actions_opp=action_opp_list,
        temperature=0.3,
        json_format=True,
        seed=None,
        stop=[],
    ...
    for i in range(len(outputs_opp)):
        ...
        if output2 == action_ground_truth_opp[0] and correct_opponent == K:
            correct_opponent = i
    return [0] * (correct_player) + [1] * (K - correct_player), [0] * (
        correct_opponent
    ) + [1] * (K - correct_opponent)


async def eval_tree(
    llm: LLMPlayer, gen: int = 9, format: str = "ou", elo_str: str = "1800+"
):
    """
    ...
    for format_path, _ in zip(pbar, range(10)):
        ...
        out = await data_battle(
            ...
            json_text,
            gen,
            prompt_translate=llm.prompt_translate,
        )
        if out:
        ...
        pbar.set_description(
            f"player moves:{correct_player/max(1,total_player)*100:.2f}. Opponent moves: {correct_opponent/max(1,total_opponent)*100:.2f}"
        )
    return


async def eval_actions(
    llm: LLMPlayer, gen: int = 9, format: str = "ou", elo_str: str = "1800+"
):
    """
    ...
    for format_path, _ in zip(pbar, range(10)):
        ...
        if out:
            for player_text in json_text:
                ...
                correct_ply, correct_opp = send_to_llm_po(
                    llm,
                    "",
                    player_prompt,
                    player_action,
                ...
                #         correct_moves_opponent[i] += correct_opp[i]
                percent_ply = [
                    round(correct / max(total_moves, 1), 2)
                    for correct in correct_moves_player
                ]
                percent_opp = [
                    round(correct / max(total_moves_opp, 1), 2)
                    for correct in correct_moves_opponent
                ]
                pbar.set_description(
                    f"player moves:{percent_ply}. Opponent moves: {percent_opp}"
                )
            ...
            for player_text in json_text:
                ...
                if "None" not in opponent_action:
                    ...
                    for i in range(K):
                        correct_moves_opponent[i] += correct_opp[i]
                percent_ply = [
                    round(correct / max(total_moves, 1) * 100, 2)
                    for correct in correct_moves_player
                ]
                percent_opp = [
                    round(correct / max(total_moves_opp, 1) * 100, 2)
                    for correct in correct_moves_opponent
                ]
                pbar.set_description(
                    f"player moves:{percent_ply}. Opponent moves: {percent_opp}"
