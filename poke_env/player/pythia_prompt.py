...
def get_system_prompt(sim: LocalSim) -> str:
    if sim.battle.active_pokemon.fainted:
        return f"""
You are a pokemon battler that targets to win the pokemon battle. Your {sim.battle.active_pokemon.species} just fainted. Choose a suitable pokemon to continue the battle. Here are some tips:
...
Consider the move-type advantage of your pokemon pokemon when its speed is higher than the opposing pokemon.
        """
    if sim.battle.active_pokemon.species in sim.prompt_strategy:
        return (
            "You are a pokemon battler that targets to win the pokemon battle. You can choose to take a move or switch in another pokemon. Here are some battle tips related to your active pokemon:\n"
            + sim.prompt_strategy[sim.battle.active_pokemon.species]
        )
...
def get_current_status(sim, battle):
    ...
    if battle.active_pokemon.type_1:
        ...
        if battle.active_pokemon.type_2:
            active_type = (
                active_type + " and " + battle.active_pokemon.type_2.name.capitalize()
            )
    ...
    active_pokemon_prompt = (
        ...
        + (
            f"Status:{sim.check_status(battle.active_pokemon.status)},"
            if sim.check_status(battle.active_pokemon.status)
            else ""
        )
        + (
...
def get_status_mon(mon: Pokemon, sim: LocalSim):
    ...
    pokemon_prompt = (
        ...
        + (
            f"Status:{sim.check_status(mon.status)},"
...
def get_macro_strat(sim: LocalSim, battle: Battle) -> str:
    """
...
def get_number_turns_faint(
    ...
    # TODO: I can use the damage calculator
    _, hp_remaining, _, _, turns = sim.calculate_remaining_hp(
        mon,
        mon_opp,
        move,
        None,
        ...
        boosts2=boosts2,
        return_turns=True,
        team=sim.battle.team,
        opp_team=sim.battle.opponent_team,
    )
...
def get_status_num_turns_fnt(
    ...
    if multiple_stat_raises:
        t_new = 1 + get_status_num_turns_fnt(
            mon, move, mon_opp, sim, boosts=boosts.copy()
        )
        turns.append(t_new)
    ...
    for move_id in mon.moves:
        if move_id == move.id:
            continue
        if mon.moves[move_id].category == MoveCategory.STATUS:
            continue
        t_new = 1 + get_number_turns_faint(
            mon, mon.moves[move_id], mon_opp, sim, boosts1=boosts.copy()
        )
        turns.append(t_new)
    if len(turns) > 0:
        return np.min(turns)
...
def get_move_opp_prompt(
    ...
    def call_dmg_calc(mon: Pokemon, mon_opp: Pokemon, move: Move):
        ...
        if move.category == MoveCategory.STATUS:
            # apply stat boosting effects to see if it will KO in fewer turns
            t = 1 + get_status_num_turns_fnt(
                mon, move, mon_opp, sim, boosts=mon._boosts.copy()
            )
            move_prompt += (
                f"{move_id}: {sim.move_effect[move.id]} {t} turns to KO your pokemon\n"
            )
            # move_prompt += f'{move_id}: {sim.move_effect[move.id]}\n'
        else:
            t = 1 + get_number_turns_faint(
                mon,
                move,
                mon_opp,
                sim,
    ...
    if (
        sim.battle._data.gen == 8
        and sim.battle.opponent_can_dynamax
        and mon_opp.active
        and mon.active
    ...
    if (
        sim.battle._data.gen == 9
        and sim.battle.opponent_can_tera
        and mon_opp.active
        and mon.active
    ):
        if not mon.terastallized:
            ...
            move_prompt += (
                f"opponent's {mon_opp.species} moves if it uses 'terastallize'"
            )
            mon_opp.terastallize()
            for move_id in moves:
                if "nothing" == move_id:
...
def get_speed_prompt(mon: Pokemon, mon_opp: Pokemon, sim: LocalSim):
...
def estimate_matchup(
    sim: LocalSim, battle: Battle, mon: Pokemon, mon_opp: Pokemon, is_opp: bool = False
):
    hp_remaining = []
    hps = []
    ...
    for move_id in moves:
        ...
        if move.category == MoveCategory.STATUS:
            # apply stat boosting effects to see if it will KO in fewer turns
            t, hp = get_status_num_turns_fnt(
                mon,
                move,
                mon_opp,
                sim,
                boosts=mon._boosts.copy(),
            )
        else:
            t, hp = get_number_turns_faint(
                mon,
                move,
                mon_opp,
                sim,
...
def get_micro_strat(sim: LocalSim, battle: Battle) -> str:
    """
...
def get_avail_actions(sim: LocalSim, battle: Battle) -> str:
...
def get_gimmick_motivation(sim: LocalSim, battle: Battle):
    ...
    if gen == 8:
        ...
        if battle.can_dynamax:
            gimmick_motiviation_prompt += (
                "You are able to use ['dynamax'] this turn as well.\n"
            )
        ...
        if sim._should_dynamax(battle):
            ...
            if battle.dynamax_turns_left == 0:
                ...
                gimmick_motiviation_prompt += (
                    f"This is your last turn while 'dynamaxed'.\n"
                )
        ...
        if battle.can_tera:
            gimmick_motiviation_prompt += (
                "You are able to use ['terastallize'] this turn as well.\n"
            )
...
def prompt_translate(sim: LocalSim, battle: Battle, return_actions=False) -> str:
    # battle_prompt = get_turn_summary(sim, battle, n_turn=16) TODO
    battle_prompt = ""
    ...
    if battle.active_pokemon.fainted:  # passive switching
        ...
        system_prompt = get_system_prompt(sim)
        system_prompt = system_prompt + macro_prompt
        state_prompt = battle_prompt + micro_prompt + gimmick_prompt
        state_action_prompt = (
            action_prompt + gimmick_motivation_prompt + action_prompt_switch
        )
...
def state_translate(
    ...
    # battle_prompt = context_prompt + " Current battle state:\n" TODO
    battle_prompt = "Current battle state:\n"
    ...
    opponent_prompt = (
        ...
        + (
            f"Status:{sim.check_status(opponent_status)},"
            if sim.check_status(opponent_status)
            else ""
        )
        + (
            f"Attack:{opponent_stats['atk']},"
            if opponent_boosts["atk"] == 0
            else f"Attack:{round(opponent_stats['atk'] * sim.boost_multiplier('atk', opponent_boosts['atk']))}({opponent_boosts['atk']} stage boosted),"
        )
        + (
            f"Defense:{opponent_stats['def']},"
            if opponent_boosts["def"] == 0
            else f"Defense:{round(opponent_stats['def'] * sim.boost_multiplier('def', opponent_boosts['def']))}({opponent_boosts['def']} stage boosted),"
        )
        + (
            f"Special attack:{opponent_stats['spa']},"
            if opponent_boosts["spa"] == 0
            else f"Special attack:{round(opponent_stats['spa'] * sim.boost_multiplier('spa', opponent_boosts['spa']))}({opponent_boosts['spa']} stage boosted),"
        )
        + (
            f"Special defense:{opponent_stats['spd']},"
            if opponent_boosts["spd"] == 0
            else f"Special defense:{round(opponent_stats['spd'] * sim.boost_multiplier('spd', opponent_boosts['spd']))}({opponent_boosts['spd']} stage boosted),"
        )
        + (
            f"Speed:{opponent_stats['spe']},"
            if opponent_boosts["spe"] == 0
            else f"Speed:{round(opponent_stats['spe'] * sim.boost_multiplier('spe', opponent_boosts['spe']))}({opponent_boosts['spe']} stage boosted),"
        )
        + (f"Ability:{opponent_ability}" if opponent_ability else "")
    )
    opponent_speed = round(
        opponent_stats["spe"] * sim.boost_multiplier("spe", opponent_boosts["spe"])
    )
    ...
    opponent_move_type_damage_prompt = move_type_damage_wrapper(
        battle.opponent_active_pokemon, sim.gen.type_chart, team_move_type
    )
    ...
    active_move_type_damage_prompt = move_type_damage_wrapper(
        battle.active_pokemon, sim.gen.type_chart, opponent_type_list
    )
    speed_active_stats = active_stats["spe"]
    if speed_active_stats == None:
        speed_active_stats = 0
    active_speed = round(
        speed_active_stats * sim.boost_multiplier("spe", active_boosts["spe"])
    )
    ...
    active_pokemon_prompt = (
        f"Your current pokemon:{battle.active_pokemon.species},Type:{active_type},HP:{active_hp_fraction}%"
        + (
            f"Status:{sim.check_status(active_status)},"
            if sim.check_status(active_status)
            else ""
        )
        + (
            f"Attack:{active_stats['atk']},"
            if active_boosts["atk"] == 0
            else f"Attack:{round(active_stats['atk']*sim.boost_multiplier('atk', active_boosts['atk']))}({active_boosts['atk']} stage boosted),"
        )
        + (
            f"Defense:{active_stats['def']},"
            if active_boosts["def"] == 0
            else f"Defense:{round(active_stats['def']*sim.boost_multiplier('def', active_boosts['def']))}({active_boosts['def']} stage boosted),"
        )
        + (
            f"Special attack:{active_stats['spa']},"
            if active_boosts["spa"] == 0
            else f"Special attack:{round(active_stats['spa']*sim.boost_multiplier('spa', active_boosts['spa']))}({active_boosts['spa']} stage boosted),"
        )
        + (
            f"Special defense:{active_stats['spd']},"
            if active_boosts["spd"] == 0
            else f"Special defense:{round(active_stats['spd']*sim.boost_multiplier('spd', active_boosts['spd']))}({active_boosts['spd']} stage boosted),"
        )
        + (
            f"Speed:{active_stats['spe']}"
            if active_boosts["spe"] == 0
            else f"Speed:{round(active_stats['spe']*sim.boost_multiplier('spe', active_boosts['spe']))}({active_boosts['spe']} stage boosted),"
        )
        + (
    ...
    for i, pokemon in enumerate(battle.available_switches):
        ...
        switch_prompt += (
            ...
            + (
                f"Status:{sim.check_status(pokemon.status)}, "
                if sim.check_status(pokemon.status)
                else ""
            )
        ...
        pokemon_move_type_damage_prompt = move_type_damage_wrapper(
            pokemon, sim.gen.type_chart, opponent_type_list
        )  # for defense
...
def get_opp_move_summary(
    pokemon: Pokemon,
    seen_moves: list[Move],
...
def state_translate2(
    ...
    moves_opp_str, moves_opp_possible_str = sim.get_opponent_current_moves(
        mon=battle.opponent_active_pokemon, return_separate=True
    )
    ...
    opponent_prompt += get_opp_move_summary(
        battle.opponent_active_pokemon, moves_opp, moves_opp_possible, battle, sim
    )
