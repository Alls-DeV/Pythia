...
class LLAMAPlayer:
    ...
    def get_LLM_action(
        ...
        inputs = self.tokenizer(
            system_prompt + user_prompt + output_padding, return_tensors="pt"
    ...
    def get_LLM_action_topK_single(
        ...
        logits = torch.stack(generated_outputs.scores, dim=1).to(
            "cpu"
        ...
        for action in actions:
            ...
            action_ids = self.tokenizer(action, return_tensors="pt")[
                "input_ids"
            ].squeeze(0)
            ...
            for t, token_id in enumerate(action_ids[1:]):
                ...
                prob += token_probs[
                    token_id
                ].item()  # Multiply probabilities for each token

            action_probabilities[action] = prob

        # Sort actions by probability in descending order
        sorted_actions = sorted(
            action_probabilities.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def get_LLM_action_topK(
        ...
        player_tokens = self.tokenizer(action_player_tokens, return_tensors="pt")[
            "input_ids"
        ].squeeze(0)
        ...
        if len(actions_opp) != 0:
            ...
            for action in actions_opp:
                ...
                for t, token_id in enumerate(action_ids[2:]):
                    # print(token_id, response_ids[t])
                    token_logits = logits[
                        t + action_player_index
                    ]  # Logits for timestep t
                    token_probs = F.softmax(
                        token_logits, dim=-1
                    )  # Convert logits to probabilities
                    # print(token_probs[token_id])
        ...
        sorted_actions_opp = sorted(
            action_probabilities_opp.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def get_LLM_action_state_value(
        self,
        system_prompt,
        user_prompt,
        model,
        ...
        logits = torch.stack(generated_outputs.scores, dim=1).to(
            "cpu"
        )  # Stack along sequence to get scores for each token
        ...
        start = (response_ids[:end] == token_start).nonzero(as_tuple=True)[0][
            -1
        ].item() + 1
    ...
    def get_LLM_action_state_value_top_K(
        self,
        system_prompt,
        user_prompt,
        model,
        ...
        actions=None,
        top_K=3,
        dmg_calc_action=None,
    ):
        # first get top K actions
        actions_top_k = self.get_LLM_action_topK_single(
            system_prompt,
            user_prompt,
            model,
            actions,
