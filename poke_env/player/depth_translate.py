...
from poke_env.player.prompts import get_avail_actions
...
async def data_battle(
    llm: LLMPlayer, file, format, battle_id, json_text, gen, prompt_translate=pt
):
    ...
    # do not use tie data
    if len(battle_turns_text) < 2:
        return False
    if not (winner_username == p1_username or winner_username == p2_username):
        return False
    ...
    for username in [winner_username]:
        ...
        for mon in moves.keys():
            moves_parsed[
                ...
                move.replace(" ", "").translate(
                    str.maketrans("", "", string.punctuation)
                )
                for move in moves[mon]
            ]
        # print(moves_parsed)
        if "random" in format:
            start_line = -1
        ...
        for turn, message in enumerate(battle_turns_text):
            ...
            if sim.battle.active_pokemon.species in moves_parsed.keys():
                ...
                sim.battle._available_moves = [
                    Move(move, gen=llm_player.gen.gen) for move in move_list
                ]
            ...
            if (
                sim.battle.active_pokemon.fainted
                or len(sim.battle.available_moves) == 0
            ):
            ...
            _, state_prompt_o, _, constraint_prompt_io_o, state_action_prompt_o = (
                sim.get_opponent_prompt(state_prompt)
            )
            state_prompt_o = (
                system_prompt
                + state_prompt_o
            ...
            ###############################################
            output, output_opp = llm.tree_search_minimax(
                5, sim.battle, sim=sim, return_opp=True
            )
