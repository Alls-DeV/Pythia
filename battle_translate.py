...
def load_filtered_dataset(
    min_month=None, max_month=None, elo_ranges=None, split="train", gamemode=None
):
    """
    ...
    if elo_ranges is not None:
        dataset = dataset.filter(
            lambda example: is_in_elo_range(example["elo"], elo_ranges)
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Pokémon battles from the pokechamp dataset"
    )
    parser.add_argument(
        "--output", default="data/processed_battles.json", help="Output JSON file path"
    )
    parser.add_argument(
        "--limit", type=int, default=1000, help="Maximum number of battles to process"
    )
    parser.add_argument(
        "--min_month",
        default="March2024",
        type=str,
        help="Minimum month_year to include (e.g., 'March2025')",
    )
    parser.add_argument(
        "--max_month",
        default="March2025",
        type=str,
        help="Maximum month_year to include (e.g., 'March2025')",
    )
    parser.add_argument(
        "--elo_ranges",
        default=["1000-1199", "1200-1399", "1400-1599", "1600-1799", "1800+"],
        type=str,
        nargs="+",
    ...
    print(
        f"Loading {args.gamemode} battles from {args.min_month} to {args.max_month} with Elo {args.elo_ranges}"
    )
    ...
    for battle in tqdm(dataset, desc="Processing battles"):
        ...
        try:
            success = await process_battle_from_text(
                battle_text, args.gamemode, battle_id, json_text, gen
            )
